-- Enforce subscription_plans.max_watches when customers create Keyword Monitor watches.
...
begin
  if v_user_id is null then
    raise exception 'AUTH_REQUIRED'
      using errcode = '28000', message = 'AUTH_REQUIRED';
  end if;

  if nullif(trim(p_name), '') is null then
    raise exception 'MONITOR_NAME_REQUIRED'
      using errcode = '22023', message = 'MONITOR_NAME_REQUIRED';
  end if;
  ...
  if v_limit is not null and v_current_count >= v_limit then
    raise exception 'PLAN_WATCH_LIMIT_REACHED'
      using errcode = 'P0001', message = 'PLAN_WATCH_LIMIT_REACHED';
  end if;
