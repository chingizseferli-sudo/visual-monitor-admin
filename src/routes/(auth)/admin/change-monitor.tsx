...
function ChangeMonitorPage() {
  ...
  useEffect(() => {
    try {
      const stored = window.localStorage.getItem('visual-change-monitor-read-alerts')
  ...
  function persistReadAlertIds(nextIds: string[]) {
    ...
    try {
      window.localStorage.setItem('visual-change-monitor-read-alerts', JSON.stringify(uniqueIds))
    } catch {
  ...
  async function loadData() {
    ...
    const [sourcesRes, eventsRes, recentEventsRes, alertsRes] = await Promise.all([
      supabase
        .from('change_sources')
        .select(
          'id,name,url,domain,selector,status,interval_minutes,telegram_chat_id,next_check_at,last_checked_at,last_changed_at,last_success_at,last_error,consecutive_fail_count,content_hash,created_at,updated_at'
        )
    ...
    if (sourcesRes.error) {
      setError(`Dəyişiklik izləmələri oxunmadı: ${sourcesRes.error.message}`)
      return
    }
    if (eventsRes.error) {
      setError(`Dəyişiklik hadisələri oxunmadı: ${eventsRes.error.message}`)
      return
    }
    ...
    setSelectedSourceIds((current) => current.filter((id) => loadedSources.some((source) => source.id === id)))
  ...
  async function saveSource(nextForm = form) {
    ...
    const response = editingSource
      ? await supabase.from('change_sources').update(payload).eq('id', editingSource.id)
    ...
    toast.success(editingSource ? 'İzləmə yeniləndi.' : 'Yeni izləmə əlavə edildi.')
    setDialogOpen(false)
    setEditingSource(null)
    setForm(emptyForm)
    resetVisualPicker()
  ...
  async function deleteSources(sourceIds: string[]) {
    ...
    setDeleting(true)
    const { error: deleteError } = await supabase.from('change_sources').delete().in('id', ids)
    setDeleting(false)
    ...
    <div className='max-w-full space-y-6 overflow-x-hidden p-6'>
      <div className='flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between'>
        ...
        <div className='flex flex-wrap items-center gap-2'>
          ...
          <Button onClick={openCreateDialog}>Yeni izləmə</Button>
        </div>
      </div>
      ...
      <Card className='max-w-full overflow-hidden'>
        ...
        <CardContent className='min-w-0 overflow-hidden'>
          {loading ? (
            ...
            <div className='rounded-lg border bg-muted/30 px-4 py-8 text-center text-sm text-muted-foreground'>
              Hələ dəyişiklik monitoru üçün izləmə əlavə edilməyib.
            </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className='max-h-[96vh] w-[96vw] max-w-[96vw] overflow-y-auto sm:max-w-[96vw]'>
          <DialogHeader>
            <DialogTitle>{editingSource ? 'İzləməni redaktə et' : 'Yeni izləmə'}</DialogTitle>
            <DialogDescription>
